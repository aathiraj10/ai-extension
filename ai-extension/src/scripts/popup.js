document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    const watsonxApiKeyInput = document.getElementById('watsonxApiKey');
    ...
    const modelsByProvider = {
      ...
      openai: [
        ...
        { value: 'ft:gpt-4o-2024-08-06:internals::BVbYWbT6', label: 'Salesforce (RHLF)' }
      ],
      watsonx: [
    ...
    if (watsonxApiKeyInput) {
      watsonxApiKeyInput.addEventListener('change', (e) => {
        storage.set({ watsonxApiKey: e.target.value });
      });
    }
    ...
    function updateApiKeyVisibility(selectedModel) {
      ...
      const watsonxContainer = document.getElementById('watsonxKeyContainer');

      if (providerSelect.value === 'groq') {
        groqContainer.style.display = 'block';
        openaiContainer.style.display = 'none';
        watsonxContainer.style.display = "none";
        ...
        groqContainer.style.display = 'none';
        openaiContainer.style.display = 'none';
        watsonxContainer.style.display = "block";  
      }
    }
    
    // Load saved values
    const result = await new Promise(resolve => {
      storage.get(['groqApiKey', 'openaiApiKey', 'watsonxApiKey', 'selectedModel', 'selectedProvider'], resolve);
    });
    ...
    if (result.watsonxApiKey && watsonxApiKeyInput) {
      watsonxApiKeyInput.value = result.watsonxApiKey;
    }    
