...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  // WatsonX API proxy handler
  if (message.type === 'watsonx-api-request') {
    console.log('[background.js] WatsonX API proxy request:', message);
    fetch(message.url, {
      ...
      .then(async (response) => {
        ...
        console.log('[background.js] WatsonX API proxy response:', { status: response.status, data });
        sendResponse({ success: response.ok, data, status: response.status });
      })
      .catch((error) => {
        console.error('[background.js] WatsonX API proxy error:', error);
